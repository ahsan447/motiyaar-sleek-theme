  ...
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      handleOptionValueChange({ data: { event, target, selectedOptionValues } }) {
        ...
        let allVariants = this.querySelector('variant-selects [data-all-variant]')?.innerHTML;
        allVariants = JSON.parse(allVariants);
      ...
      updateShareUrl(variantId) {
        ...
        toggleAddButton(disable = true, text, modifyClass = true) {
          ...
          // Get current variant from allVariants
          const currentVariantData = this.allVariants?.find((variant) => variant.id === this.currentVariant?.id);
          console.log('Current Variant Data:', currentVariantData);

          if (disable) {
            addButton.setAttribute('disabled', 'disabled');
            if (text) {
              // Check if variant has continue selling policy
              if (currentVariantData && currentVariantData.inventory_policy === 'continue') {
                console.log('Setting Preorder text - Policy is continue');
                addButtonText.textContent = 'Preorder';
                addButton.removeAttribute('disabled');
              } else {
                console.log('Setting original text:', text);
